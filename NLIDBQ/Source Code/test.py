...
while (len(newString)>1):
	...
	if whereFlag==1:		
		...
		if tables.has_key(newString) == True:
			#print tables[newString]
			print "\nMatch found in dictionary for : " + newString + " corresponds to : " + tables[newString]
			print "Removing : " + newString + "\n"
			table_names.append(tables[newString])				#Used to add to Table dict
			newString = removeWords(nlQuery,newString)
			nlQuery = newString
			print "New string is now : " + nlQuery + "\n"
			continue
